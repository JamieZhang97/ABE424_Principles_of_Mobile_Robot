...
class listenerNode():
    ...
    def EKF(self):
        ...
        pydot = self.xhat[4,0]
        pzdot = self.xhat[5,0]
        vdot=np.dot(self.L_lb,np.array([[self.fx],[self.fy],[self.fz]]))
        vxdot = vdot[0,0]
        vydot = vdot[1,0]
        vzdot = vdot[2,0]
        q1dot= -0.5*(self.p*self.q2+self.q*self.q3+self.r*self.q4)
        q2dot= -0.5*(-self.p*self.q1-self.r*self.q3+self.q*self.q4)
        q3dot= -0.5*(-self.q*self.q1+self.r*self.q2-self.p*self.q4)
        q4dot= -0.5*(-self.r*self.q1-self.q*self.q2+self.p*self.q3)
        
        #Now integrate Euler Integration for Process Updates and Covariance Updates
        # Euler works fine
        self.xhatdot=np.array([[pxdot, pydot, pzdot, vxdot, vydot, vzdot, q1dot, q2dot, q3dot, q4dot, 0, 0, 0, 0, 0, 0]]).T
        self.xhat=self.xhat+self.xhatdot*self.dt
        ...
        #delV/delQ
        vqdot=np.array([[2*(self.q1*self.fx-self.q4*self.fy+self.q3*self.fz), 2*(self.q2*self.fx+self.q3*self.fy+self.q4*self.fz), 2*(-self.q3*self.fx+self.q2*self.fy+self.q1*self.fz), 2*(-self.q4*self.fx-self.q1*self.fy+self.q2*self.fz)], 
                        [-2*(-self.q4*self.fx-self.q1*self.fy+self.q2*self.fz), -2*(-self.q3*self.fx+self.q2*self.fy+self.q1*self.fz), 2*(self.q2*self.fx+self.q3*self.fy+self.q4*self.fz), 2*(self.q1*self.fx-self.q4*self.fy+self.q3*self.fz)], 
                        [2*(-self.q3*self.fx+self.q2*self.fy+self.q1*self.fz), -2*(-self.q4*self.fx-self.q1*self.fy+self.q2*self.fz), -2*(self.q1*self.fx-self.q4*self.fy+self.q3*self.fz), 2*(self.q2*self.fx+self.q3*self.fy+self.q4*self.fz)]])

        #delV/del_abias
        vbadot=-self.L_lb
        #delQ/delQ
        qqdot=-0.5*np.array([[0, self.p, self.q, self.r],
                             [-self.p, 0, -self.r, self.q],
                             [-self.q, self.r, 0, -self.p],
                             [-self.r, -self.q, self.p, 0]])
        #delQ/del_gyrobias
        qbwdot=0.5*np.array([[ self.q2, self.q3, self.q4],\
                             [-self.q1, self.q4, -self.q3],\
                             [-self.q4, -self.q1, self.q2],\
                             [self.q3, -self.q2, -self.q1]])
        # Now assemble the Transition matrix
        Phi=np.block([[np.zeros([3,3]), np.eye(3), np.zeros([3,4]), np.zeros([3,3]), np.zeros([3,3])],\
                      [np.zeros([3,3]), np.zeros([3,3]), vqdot, np.zeros([3,3]), vbadot],\
                      [np.zeros([4,3]), np.zeros([4,3]), qqdot, qbwdot, np.zeros([4,3])],\
                      [np.zeros([3,3]), np.zeros([3,3]), np.zeros([3,4]), np.zeros([3,3]), np.zeros([3,3])],\
                      [np.zeros([3,3]), np.zeros([3,3]), np.zeros([3,4]), np.zeros([3,3]), np.zeros([3,3])]])
        ...
        Pdot = Phi@self.P+self.P@Phi.transpose() + self.Q
        #P1 = Pdot*dt
        #P = P +Pdot*dt
        ...
        rgps = np.array([[1.5,0,0]])
        # del v/del q
        Hxq = np.array([[-rgps[0,0]*2*self.q1, -rgps[0,0]*2*self.q2, rgps[0,0]*2*self.q3, rgps[0,0]*2*self.q4],
                      [-rgps[0,0]*2*self.q4, -rgps[0,0]*2*self.q3, -rgps[0,0]*2*self.q2, -rgps[0,0]*2*self.q1],
                      [rgps[0,0]*2*self.q3, -rgps[0,0]*2*self.q4, rgps[0,0]*2*self.q1, -rgps[0,0]*2*self.q2]])
        #del P/del q
        Hvq = np.array([[rgps[0,0]*2*self.q3*self.q + rgps[0,0]*2*self.q4*self.r,  rgps[0,0]*2*self.q4*self.q - rgps[0,0]*2*self.q3*self.r, rgps[0,0]*2*self.q1*self.q - rgps[0,0]*2*self.q2*self.r, rgps[0,0]*2*self.q2*self.q + rgps[0,0]*2*self.q1*self.r],
                      [-rgps[0,0]*2*self.q2*self.q + rgps[0,0]*2*self.q1*self.r,  rgps[0,0]*2*self.q2*self.r - rgps[0,0]*2*self.q1*self.q, rgps[0,0]*2*self.q4*self.q - rgps[0,0]*2*self.q3*self.r, rgps[0,0]*2*self.q3*self.q + rgps[0,0]*2*self.q4*self.r],
                      [rgps[0,0]*2*self.q1*self.q - rgps[0,0]*2*self.q2*self.r,  -rgps[0,0]*2*self.q2*self.q + rgps[0,0]*2*self.q1*self.r, -rgps[0,0]*2*self.q3*self.q - rgps[0,0]*2*self.q4*self.r, rgps[0,0]*2*self.q4*self.q - rgps[0,0]*2*self.q3*self.r]])
#       
        
        # Assemble H
        self.H = np.block([[np.eye(3), np.zeros([3,3]), Hxq, np.zeros([3,6])],\
                         [np.zeros([3,3]), np.eye(3), Hvq, np.zeros([3,6])]])
        #Compute Kalman gain
        e = self.z - np.dot(self.H, self.xhat)
        S = np.dot(np.dot(self.H, self.P), self.H.T) + self.R
        K = np.dot(np.dot(self.P, self.H.T), np.inv(S))
        #Perform xhat correction    xhat = xhat + K@(z-H@xhat)
        self.xhat = self.xhat + np.dot(K,e)
        #propagate error covariance approximation P = (np.eye(16,16)-K@H)@P
        self.P = np.dot(np.eye(16) - np.dot(K,self.H),self.P)
        #end
